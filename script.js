...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function addSalesItemRow() {
        ...
        const itemWrapper = document.createElement('div'); // Perubahan di sini: sales-item-wrapper
        itemWrapper.classList.add('sales-item-wrapper'); // Menggunakan kelas baru
        itemWrapper.innerHTML = `
            <div class="sales-item-header">
                <span class="sales-item-title">Item Penjualan Baru</span>
            </div>
        ...
        `;
        salesItemsContainer.appendChild(itemWrapper); // Append itemWrapper
        M.FormSelect.init(itemWrapper.querySelectorAll('select')); // Inisialisasi select di dalam itemWrapper
        M.updateTextFields(); // Perbarui field teks Materialize

        itemWrapper.querySelector('.category-select').addEventListener('change', function () {
            const selectedCategory = this.value;
            const katabanSelect = itemWrapper.querySelector('.kataban-select');
        ...
        itemWrapper.querySelector('.remove-item-btn').addEventListener('click', function () {
            itemWrapper.remove();
        });
    }
    ...
    submitReportBtn.addEventListener('click', async function (e) {
        ...
        const reportItems = [];
        // Mengubah selector menjadi .sales-item-wrapper
        const itemWrappers = salesItemsContainer.querySelectorAll('.sales-item-wrapper'); 

        if (!reportDate || !storeName || !spmName) return M.toast({ html: 'Lengkapi data wajib!', classes: 'red' });
        if (!isNoSale && itemWrappers.length === 0) return M.toast({ html: 'Tambahkan minimal 1 item atau centang No Sale', classes: 'red' });
        ...
        if (!isNoSale) {
            for (const wrapper of itemWrappers) { // Mengubah variabel iterasi
                const category = wrapper.querySelector('.category-select').value;
                const model = wrapper.querySelector('.kataban-select').value;
